...
class AnalyticsManager: ObservableObject {
    ...
    deinit {
        ...
        // Note: Cannot call async method in deinit, events will be flushed on next app launch
    }
    ...
    private func sendEvents(_ events: [AnalyticsEvent]) async {
        ...
        // Working mode: Try to send to analytics endpoint, gracefully handle 404
        do {
            let request = AnalyticsRequest(events: events)
            // FIX: Ensure leading slash so APIClient builds a valid URL
            _ = try await apiClient.post("/analytics/events", body: request) as AnalyticsResponse
            
            logger.info("✅ Analytics events sent successfully")
            
        } catch APIClientError.serverError(404, let _) {
            // Analytics endpoint not available - log locally for development
            logger.info("📊 Analytics endpoint not available, logging \(events.count) events locally")
            ...
            // Don't re-queue 404 errors to avoid endless retries
            
        } catch {
            logger.error("❌ Failed to send analytics events: \(error)")
            
            // Re-queue other errors for retry (with limit to prevent infinite growth)
    ...
    func trackUserAction(_ action: String, parameters: [String: Any] = [:]) {
        track(action, properties: parameters)
    }
    
    /// Backward compatibility method for existing code
    func trackLearningProgress(_ event: String, parameters: [String: Any] = [:]) {
        track(event, properties: parameters)
    }
}
