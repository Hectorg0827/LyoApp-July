...
class ContentPlayerService: NSObject, ObservableObject {
    ...
    func loadContent(_ resource: LearningResource) async {
        ...
        do {
            guard let contentURL = resource.contentURL else {
                throw ContentPlayerError.invalidURL
            }
            
            // Check if content is downloaded locally
            if let localPath = await getLocalContentPath(for: resource.id.uuidString) {
    ...
    private func saveProgress(for content: LearningResource, time: TimeInterval, completed: Bool = false) async {
        do {
            ...
            try await dataService.updateProgress(resourceId: content.id.uuidString, userId: "current_user", progress: progress)
        } catch {
