...
class BackendIntegrationService: ObservableObject {
    ...
    // MARK: - Data Cache
    @Published var cachedFeedItems: [FeedItem] = []
    ...
    func authenticateUser(email: String, password: String) async throws -> User {
        ...
        do {
            ...
            let user = User(
                ...
                level: response.user.isVerified ? 5 : 1
            )
    ...
    func loadFeedContent(page: Int = 1, limit: Int = 20, refresh: Bool = false) async throws -> [FeedItem] {
        logger.info("📰 Loading feed content (page: \(page), limit: \(limit), refresh: \(refresh))")
        
        if refresh {
            cachedFeedItems.removeAll()
        }
        ...
        do {
            let feedItems = try await apiClient.loadFeed(page: page, limit: limit)
            
            if page == 1 || refresh {
                cachedFeedItems = feedItems
            } else {
                cachedFeedItems.append(contentsOf: feedItems)
            }
            
            logger.info("✅ Loaded \(feedItems.count) feed posts")
            return feedItems
    ...
    func loadLearningResources(query: String? = nil, limit: Int = 20, refresh: Bool = false) async throws -> [Course] {
        ...
        do {
            let courses = try await apiClient.fetchLearningResources(query: query, limit: limit)
            
            if refresh {
                cachedCourses = courses
            } else {
                // Avoid duplicates when appending
                let newCourses = courses.filter { newCourse in
                    !cachedCourses.contains { $0.id == newCourse.id }
                }
                cachedCourses.append(contentsOf: newCourses)
            }
            
            logger.info("✅ Loaded \(courses.count) learning resources")
            return courses
    ...
    func getConnectionDiagnostics() -> [String: Any] {
        return [
            ...
            "cachedFeedItems": cachedFeedItems.count,
            "cachedCourses": cachedCourses.count,
