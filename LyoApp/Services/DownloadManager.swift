...
class DownloadManager: ObservableObject {
    ...
    private func downloadResourceContent(_ resource: LearningResource) async {
        ...
        activeDownloads[resource.id] = downloadTask
        
        // For now, skip actual downloading since API model doesn't have contentURL
        // TODO: Implement proper file downloading when API provides file URLs
        print("📱 Download queued for: \(resource.title)")
        
        // Simulate download completion
        DispatchQueue.main.asyncAfter(deadline: .now() + 1.0) {
            self.activeDownloads.removeValue(forKey: resource.id)
            print("✅ Download completed for: \(resource.title)")
        }
    }
