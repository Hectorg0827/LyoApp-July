...
final class APIClient: ObservableObject {
    ...
    func login(email: String, password: String) async throws -> LoginResponse {
        ...
        currentUser = user
        setAuthToken(response.actualAccessToken, refreshToken: response.refreshToken, userId: user.id)
    ...
    // MARK: - Search API
    func searchUsers(_ query: String, limit: Int = 20) async throws -> SearchUsersResponse {
    ...
    func searchContent(_ query: String, limit: Int = 20) async throws -> SearchContentResponse {
        let params = ["q": query, "limit": String(limit)]
