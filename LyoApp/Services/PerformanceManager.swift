...
private class ImageLoader: ObservableObject {
    ...
    func load(from url: URL?) {
        ...
        // Check cache first
        if let cachedImage = performanceManager.getCachedImage(forKey: urlString) {
            image = cachedImage
            return
        }
        ...
        currentTask = Task {
            do {
                ...
                if let downloadedImage = UIImage(data: data) {
                    // Cache the image
                    performanceManager.cacheImage(downloadedImage, forKey: urlString)
                ...
                // Handle error silently - placeholder will remain visible
            }
        }
    }
