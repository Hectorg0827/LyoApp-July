import SwiftUI
// Using ChatMessage defined in TypeDefinitions.swift (canonical). Duplicate import removed.
        ...
        Button(action: action) {
            VStack(spacing: DesignTokens.Spacing.xs) {
                ZStack {
                    ...
                    AsyncImage(url: story.author.profileImageURL ?? URL(string: "https://picsum.photos/60/60")!) { image in
                        image
                            .resizable()
...
struct StoryViewerView: View {
    ...
    var body: some View {
        VStack(spacing: DesignTokens.Spacing.md) {
            ...
            HStack(spacing: DesignTokens.Spacing.sm) {
                AsyncImage(url: story.author.profileImageURL ?? URL(string: "https://picsum.photos/40/40")!) { image in
                    image
                        .resizable()
                        .aspectRatio(contentMode: .fill)
                } placeholder: {
...
struct EnhancedStoryViewer: View {
    ...
    private func topOverlay(currentStory: Story) -> some View {
        VStack(spacing: 12) {
            ...
            HStack {
                AsyncImage(url: currentStory.author.profileImageURL ?? URL(string: "https://via.placeholder.com/40")!) { image in
                    image
                        .resizable()
                        .aspectRatio(contentMode: .fill)
                } placeholder: {
...
// MARK: - Progress Bar Component (Legacy)
struct LegacyProgressBar: View {
    let progress: Double
