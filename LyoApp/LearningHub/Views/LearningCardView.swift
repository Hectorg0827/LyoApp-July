import SwiftUI
// Standard imports only; removed invalid @_exported imports which caused build errors.
...
private extension LearningCardView {
    ...
    Text(resource.resourcePlatform?.displayName ?? "Unknown")
            .font(.system(size: 9, weight: .medium))
...
#Preview("Standard Card") {
    let sampleData = LearningResource.sampleResources()
    LearningCardView(
        resource: sampleData[0],
        cardStyle: .standard,
        onTap: {}
    )
    .padding()
