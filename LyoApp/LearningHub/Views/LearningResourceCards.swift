import SwiftUI
// Uses LegacyLearningResource (canonical during migration) instead of conflicting LearningResource variants.
...
#if DEBUG
#Preview {
    // Use legacy sample resources explicitly to avoid type mismatches during migration.
    let samples = LearningResource.sampleResources()
    ScrollView(.horizontal, showsIndicators: false) {
        HStack(spacing: 20) {
            ForEach(samples.prefix(3)) { res in
                LearningResourceFeaturedCard(resource: res)
            }
        }
        .padding()
    }
