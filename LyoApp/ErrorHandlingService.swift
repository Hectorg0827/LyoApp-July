...
class ErrorHandlingService: ObservableObject {
    ...
    private func checkNetworkConnection() async {
        // Simulated connectivity check (replace with real monitor later)
        // TODO: Implement real network monitoring
        print("Network connection status => Connected")
    }
    ...
    private func checkServerStatus() async {
        ...
        do {
            ...
            let healthResponse = try await APIClient.shared.getSystemHealth()
            let isOperational = healthResponse.status == "healthy"
            print("Server is \(isOperational ? "operational" : "having issues")")
        } catch {
    ...
    private func checkConnection() async {
        // Use the specific method available
        do {
            let isConnected = try await APIClient.shared.checkConnection()
            print("Connection check: \(isConnected ? "Connected" : "Disconnected")")
        } catch {
...
struct LyoError: Error, Identifiable {
    ...
    static func from(_ error: Error, context: ErrorContext? = nil) -> LyoError {
        ...
        case let wsError as LyoWebSocketError:
            return LyoError(
...
enum ErrorType {
    ...
    case webSocketError(LyoWebSocketError)
    case voiceRecognitionError
    case courseGenerationError
    case lessonContentError
    case authenticationError
...
extension ErrorHandlingService {
    ...
    func performWebSocketOperation<T>(
        ...
        do {
            return try await operation()
        } catch let error as LyoWebSocketError {
...
// MARK: - Supplementary Handlers
extension ErrorHandlingService {
    /// Handles web socket specific errors with additional logging or recovery if needed
    private func handleWebSocketError(_ error: LyoWebSocketError) {
        let context = ErrorContext(
            operation: "WebSocket",
            endpoint: "Realtime Channel",
            userAction: "Live interaction"
        )
        handleError(error, context: context)
    }
}
